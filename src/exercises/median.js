...
const minHeap = () => {
  ...
  const minChildIndex = parent => {
    ...
    if (left < heapSize && right < heapSize) {
      return nodes[left].getKey() < nodes[right].getKey() ? left : right;
  ...
  const bubbleLast = () => {
    ...
    while (child > 0 && nodes[parent].getKey() > nodes[child].getKey()) {
      swap(child, parent);
      child = parent;
...
function cookies(k, A) {
  const heap = minHeap();
  A.forEach(v => heap.insert(v, v));

  let nrOfOperations = 0;
  while (heap.size() > 2 && heap.min() < k) {
    const first = heap.extractMin();
    const second = heap.extractMin();
    const newCookie = 1 * first + 2 * second;
    heap.insert(newCookie, newCookie);
    nrOfOperations += 1;
  }
...
console.log(cookies(6, [1, 2, 3, 9, 10, 12]));
